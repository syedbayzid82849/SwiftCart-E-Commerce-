...
const displayCategories = (categories) => {
    ...
    categories.forEach(cat => {
        categoryButtons.innerHTML += `<button data-cat="${cat}" class="cat-btn px-5 py-2 rounded-full text-sm font-semibold border border-gray-200 bg-white text-gray-700 capitalize" id=" onclick="filterByCategory('${cat}')">${cat}</button>`;
    });
};
