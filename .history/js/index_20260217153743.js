...
const displayCategories = (categories) => {
    ...
    categories.forEach(cat => {
        categoryButtons.innerHTML += `<button id="cat-btn-${cat}" data-cat="${cat}" class="cat-btn px-5 py-2 rounded-full text-sm font-semibold border border-gray-200 bg-white text-gray-700 capitalize" onclick="filterByCategory('${cat}')">${cat}</button>`;
    });
};
